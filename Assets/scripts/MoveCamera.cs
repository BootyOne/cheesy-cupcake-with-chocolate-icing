...
public class MoveCamera : MonoBehaviour
{
   ...
   // [SerializeField] private GameObject MovedDecor;

    private Transform player;
    private Vector3 min, max, direction;
    private static MoveCamera _use;
    private Camera cam;

    public static MoveCamera use
    {
        get { return _use; }
    }
    ...
    void Follow()
    {
        ...
        transform.position = Vector3.Lerp(transform.position, position, smooth); //для плавности smooth * Time.deltaTime
       //MovedDecor.transform.position = Vector3.Lerp(MovedDecor.transform.position, position, smooth); //для плавности smooth * Time.deltaTime
       // MovedDecor.transform.position = transform.position;
    }
